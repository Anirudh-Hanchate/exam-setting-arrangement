...
def generate_allotment_api():
    ...
    for branch in branch_details:
        try:
            ...
            if start > end:
                return jsonify({'status': 'error', 'message': f'For branch {name}, start USN > end USN.'}), 400
    ...
    for group in common_groups:
        ...
        if group_leader not in defined_branch_names:
            return jsonify({'status': 'error', 'message': f"Common Group Error: The group leader '{group_leader}' is not a defined branch."}), 400

        merged_list = []
        for branch_name in group:
            if branch_name not in defined_branch_names:
                return jsonify({'status': 'error', 'message': f"Common Group Error: The branch '{branch_name}' in group '{group_leader}' is not defined."}), 400
    ...
    for branch_name, students in initial_student_lists.items():
        if branch_name not in processed_in_group:
            final_student_lists[branch_name] = students
